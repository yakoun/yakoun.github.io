  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-950 flex pb-16 lg:pb-0">
    ...
    {#if mobileNavOpen}
      ...
      <div role="presentation" class="fixed inset-0 z-40 lg:hidden" on:click={() => mobileNavOpen = false}>
        ...
        <!-- svelte-ignore a11y-click-events-have-key-events a11y-no-static-element-interactions -->
        <div role="dialog" aria-modal="true" class="absolute bottom-0 left-0 right-0 bg-white dark:bg-gray-900 rounded-t-2xl max-h-[85vh] overflow-y-auto p-5 shadow-xl safe-area-bottom" on:click|stopPropagation>
          <div class="w-10 h-1 rounded-full bg-gray-300 dark:bg-gray-600 mx-auto mb-4" />
            <div class="space-y-0.5">
              {#each navItems as item}
                <a
                  ...
                  class="flex items-center gap-3 px-3 py-3.5 rounded-xl text-sm font-medium transition-all active:scale-[0.98] {$page.url.pathname === item.href ? 'bg-primary-50 dark:bg-primary-900/50 text-primary-700 shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'}"
                >
                <svelte:component this={item.icon} size={18} />
                {item.label}
              </a>
            {/each}
              ...
              <button on:click={handleLogout} class="flex items-center gap-3 px-3 py-3.5 rounded-xl text-sm font-medium text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 w-full transition-all active:scale-[0.98]">
              <LogOut size={18} /> Déconnexion
            </button>
          </div>
        </div>
    ...
    <div class="lg:pl-64 flex-1 min-w-0">
      ...
      <main class="p-3 sm:p-5 lg:p-8 w-full max-w-full overflow-x-hidden">
        <slot />
      </main>
    </div>

    <!-- Mobile bottom tab bar -->
    <nav class="fixed bottom-0 left-0 right-0 z-30 lg:hidden bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 flex items-center justify-evenly gap-0.5 px-1 safe-area-bottom">
      {#each quickItems as item}
        <a
          href={item.href}
          class="flex flex-col items-center gap-0.5 py-2.5 px-1 text-[9px] sm:text-[10px] font-medium min-w-0 {$page.url.pathname === item.href ? 'text-primary-600' : 'text-gray-400'} transition-colors"
        >
          <svelte:component this={item.icon} size={18} />
          <span class="truncate max-w-14 text-center leading-tight">{item.label}</span>
        </a>
      {/each}
      <button
        on:click={() => mobileNavOpen = !mobileNavOpen}
        class="flex flex-col items-center gap-0.5 py-2.5 px-1 text-[9px] sm:text-[10px] font-medium text-gray-400 transition-colors min-w-0"
      >
        <Menu size={18} />
        <span>Plus</span>
      </button>
