<script lang="ts">
  ...
  import { Pencil, Trash2, Plus, Star, Save } from 'lucide-svelte'
  ...
  async function save() {
    try {
      if (editing) await supabase.from('gallery').update(form).eq('id', editing.id)
      else await supabase.from('gallery').insert(form)
      reset(); load()
    } catch (e: any) { alert(e?.message) }
  }

  function edit(it: any) { editing = it; form = { title: it.title || '', description: it.description || '', image_url: it.image_url, category: it.category || '', featured: it.featured }; showForm = true }
  ...
  function reset() { showForm = false; editing = null; form = { title: '', description: '', image_url: '', category: '', featured: false } }
</script>
...
{#if showForm}
  <div class="card mb-6 space-y-3">
    ...
    <div><label class="label">URL de l'image <input type="url" bind:value={form.image_url} required class="input" placeholder="https://..." /></label></div>
...
<div class="grid md:grid-cols-3 gap-4">
  {#each items as item}
    <div class="card p-3">
      <img src={item.image_url} alt={item.title} class="w-full h-40 object-cover rounded-xl mb-3" />
