<script lang="ts">
  import { supabase, isConfigured, getCertifications } from '$lib/api/supabase'
  import { onMount } from 'svelte'
  import { Plus, Pencil, Trash2, Save, X, Award, AlertTriangle, Database, ExternalLink } from 'lucide-svelte'
  ...
  async function createTable() {
    ...
    try {
      ...
      if (data.ok) {
        setupMessage = 'Table créée avec succès ! Rechargement...'
        setTimeout(() => { creatingTable = false; load() }, 1000)
      } else {
...
{#if !tableExists}
  <div class="flex items-start gap-3 p-4 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 mb-6">
    ...
    <div class="text-sm text-amber-700 dark:text-amber-400 flex-1">
      ...
      <div class="mt-3 flex flex-wrap gap-2">
        <button on:click={createTable} disabled={creatingTable} class="btn-primary text-sm flex items-center gap-1.5">
          <Database size={14} /> {creatingTable ? 'Création en cours...' : 'Créer la table automatiquement'}
        </button>
        <a href="https://supabase.com/dashboard/project/aqcpfjoeczyuxvqodlth/sql/new" target="_blank" rel="noopener" class="btn-secondary text-sm flex items-center gap-1.5">
          <ExternalLink size={14} /> Ouvrir SQL Editor
        </a>
      </div>
      {#if setupMessage}
        <p class="mt-3 text-xs {setupMessage.includes('succès') ? 'text-green-600' : 'text-amber-600'}">{setupMessage}</p>
      {/if}
      {#if setupSQL}
        <pre class="mt-3 p-3 rounded-lg bg-gray-900 text-gray-100 text-xs overflow-x-auto whitespace-pre-wrap">{setupSQL}</pre>
      {/if}
    </div>
  </div>
{:else if loading}
